...
We propose to leverage the framework described by~\citet{Rana20corl}, to learn a controller from potentially suboptimal demonstrations. This is accomplised by learning the Riemannian Motion Policy (RMP) potential functions $\Phi$ and the corresponding Riemannian metric \textbf{M}.
...
We will compare our framework to the one proposed in~\citet{Rana20corl}. A simple metric we can use is ratio of task success as well as time for task execution.
