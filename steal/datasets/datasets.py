...
class PullbackDataset(Dataset):
    ...
    def __getitem__(self, index):
        x_batch = {
            ...
            'psi_list':
            [psi[index]
             for psi in self.psi_list] if self.psi_list is not None else None,
            'J_list':
            [J[index]
             for J in self.J_list] if self.J_list is not None else None,
            'Jd_list':
            [Jd[index]
             for Jd in self.Jd_list] if self.Jd_list is not None else None,
            'dphi_list': [dphi[index] for dphi in self.dphi_list]
            if self.dphi_list is not None else None
        }
