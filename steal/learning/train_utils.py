...
def train_dagger(model,
    ...
    assert batch_size is None, NotImplementedError(
        'Doesnt work for minibatches!')

    batch_size = len(concat_dataset)

    p = 0.05**(
        1. / (n_dagger_iterations - 1 + 1e-14)
    )  # the probabilty at the last iteration should be low (0.05 here)
    ...
    learner_time_dependent = TimeWrapperNet(
        model
    )  # just adding in an additional dummy time variable to the learner model
    mixture_model = MixtureNet(model_1=experts[0],
                               model_2=learner_time_dependent,
                               beta=1.)
    ...
    for i in range(n_dagger_iterations):
        ...
        if i == 0:
            ...
            print('------------ mixing_rate: {}, tracker_kp: {} ----------'.
                  format(beta, tracker_kp))
            print('----------------------------------------------------------')
            for n in range(n_experts):
                s0 = s0_list[n]
                ...
                x_n = generate_trajectories(mixture_model,
                                            s0,
                                            time_dependent=True,
                                            order=1,
                                            return_label=False,
            ...
            data_loader_expert = DataLoader(dataset_expert,
                                            batch_size=batch_size_expert,
                                            shuffle=True)
            data_loader_new = DataLoader(dataset_new,
        ...
        train(model=model,
              ...
              clip_value_weight=clip_value_weight,
              log_freq=log_freq,
              logger=logger,
              loss_clip=loss_clip,
        ...
        for n in range(n_experts):
            ...
            t_eval = expert_time_list[n]
            t_final = t_eval[-1]
            learner_traj = generate_trajectories(model,
                                                 s0,
            ...
            traj_mse = traj_mse + traj_criterion(learner_traj,
                                                 expert_traj_list[n]).item()
        ...
        print('Best Trajectory MSE: {}, Best Iteration: {}'.format(
            best_mse, best_iteration))
...
def train_dagger_context(context_model,
                         loss_fn,
                         opt,
                         context_dataset,
                         dt,
                         context_rmptree_list,
                         subtask_maps,
                         n_epochs=500,
                         batch_size=None,
                         shuffle=True,
    ...
    assert len(context_rmptree_list) == n_experts, ValueError(
        'Rmptree list should have length equal to number of demos!')
    ...
    for n in range(n_experts):
        ...
        expert_model = TrajectoryTrackingController(q_expert.numpy(),
                                                    qd_expert.numpy(),
                                                    dt=dt,
                                                    kp=tracker_kp,
                                                    kd=1.0)
    ...
    for context_rmptree in context_rmptree_list:
        ...
        for subtask_map in subtask_maps:
            subtask_node = context_rmptree.add_task_space(subtask_map,
                                                          name='subtask_' +
                                                          str(n))
            subtask_net = subtask_nets[n + 1]
            subtask_momentum_controller = LearnedMomentumController(
                momentum_net=subtask_net, metric_scaling=1.)
    ...
    for i in range(n_dagger_iterations):
        ...
        if i == 0:
            ...
            for n in range(n_experts):
                ...
                for i in range(N):
                    ...
                    qd_learner = learner.eval_canonical(t=i * dt,
                                                        s=State(q.T,
                                                                order=1)).xd.T
                    ...
                    context_rmp_eval = context_rmptree.eval_natural(
                        t=i * dt, s=State(q.T, order=1))
                    p[i] = context_rmp_eval.p.T
                    m[i] = context_rmp_eval.M

                x_list.append(torch.from_numpy(q_traj).float())
                J_list.append(
                    torch.eye(cspace_dim, dtype=torch.float).repeat((N, 1, 1)))
                ...
                for subtask_map in subtask_maps:
                    ...
                    for n in range(N):
                        x[n] = subtask_map.psi(q_traj[n].reshape(-1,
                                                                 1)).flatten()
                ...
                dataset = ContextDatasetMomentum(
                    ...
                    torch.from_numpy(qd_traj_expert).float(), x_list, J_list,
                    p_list, m_list)
            ...
            data_loader_expert = DataLoader(context_dataset,
                                            batch_size=batch_size_expert,
                                            shuffle=True)
            data_loader_new = DataLoader(context_dataset_new,
                                         batch_size=batch_size_new,
                                         shuffle=True)
        ...
        context_model.train()
        best_model, best_mse = train(model=context_model,
                                     loss_fn=loss_fn,
                                     opt=opt,
                                     train_dataset=dataset,
                                     n_epochs=n_epochs,
...
class MixtureNet(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        return float(alpha) * self.model_1(
            *args, **kwargs) + (1. - alpha) * self.model_2(*args, **kwargs)
...
class TrajectoryTrackingTimeDependentController(nn.Module):
    ...
    def forward(self, t, x):
        ...
        xdd = -self.kp * (x - self.x_traj[idx]) - self.kd * (0.0 -
                                                             self.xd_traj[idx])
        return xdd
