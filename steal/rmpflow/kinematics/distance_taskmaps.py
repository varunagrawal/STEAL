...
class BoxDistanceTaskMap(TaskMap):
    ...
    def __init__(self,
        ...
        if box_center is None:
            ...
            self.box_center = box_center.to(device=self.device,
                                            dtype=torch.get_default_dtype())

        if box_size is None:
            self.box_size = 2. * torch.ones(1, 2, device=self.device)
        else:
            self.box_size = box_size.to(device=self.device,
                                        dtype=torch.get_default_dtype())
        ...
        super(BoxDistanceTaskMap, self).__init__(n_inputs=self.n_inputs,
                                                 n_outputs=1,
                                                 psi=psi,
                                                 device=self.device)
