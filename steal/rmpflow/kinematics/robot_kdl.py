...
class Robot(object):
    ...
    def __init__(self, in_joint_names=None, urdf_path=None, workspace_dim=3):
        ...
        if self.in_joint_names is None:
            print(
                colored(
                    'WARNING: Joint name mapping not provided. Using kdl order!',
                    'green', 'on_red'))
    ...
    def sort_link_names(self):
        ...
        for i in range(len(self.link_names)):
            num_segments = int(
                self.kdl_tree.getChain(self.base_link,
                                       self.link_names[i]).getNrOfSegments())
    ...
    def get_task_map(self,
        ...
        if self.in_joint_names is None:
            ...
            np_to_kdl_idx_map = self.find_np_to_kdl_map(
                kdl_chain, self.in_joint_names)
        ...
        def jacobian(q):
            ...
            jacobian = torch.zeros(self.workspace_dim,
                                   self.num_joints,
                                   device=device)
        ...
        def jacobian_dot(q, q_dot):
            ...
            kdl_jacobian_dot.JntToJacDot(self.torch_to_kdl_vel(q_, qd_),
                                         jacobian_dot_)
            jacobian_dot = torch.zeros(self.workspace_dim,
                                       self.num_joints,
                                       device=device)
        ...
        link_task_map = TaskMap(n_inputs=self.cspace_dim,
                                n_outputs=self.workspace_dim,
                                psi=batch_forward_kinematics,
                                J=batch_jacobian,
                                J_dot=batch_jacobian_dot,
                                device=device)
...
if __name__ == '__main__':
    filename = os.path.abspath(
        os.path.join(os.path.dirname(__file__),
                     '../../urdf/point_planar_robot.urdf'))
    np_joint_names = ['joint1', 'joint2', 'joint3']
    robot = Robot(urdf_path=filename,
                  workspace_dim=2)  #, in_joint_names=np_joint_names)
