...
class ObstacleAvoidanceForceController(NaturalGradientDescentForceController):
    ...
    def __init__(self,
        ...
        if alpha_switch is None:  #sets a hard velocity based toggle switch
            ...
            G = BarrierWithVelocityToggleSoftMetric(epsilon=epsilon,
                                                    alpha=alpha_switch,
                                                    device=device)

        Phi = BarrierPotential(proportional_gain=proportional_gain,
                               device=device)
        del_Phi = Phi.grad

        def B(x, xd):
            return damping_gain * G(x, xd)  # such that inv(G)*B = constant

        super(ObstacleAvoidanceForceController, self).__init__(G=G,
                                                               B=B,
                                                               del_Phi=del_Phi,
                                                               ds_type=ds_type,
                                                               device=device)
...
# ---------------------------------------------------------------------
class ObstacleAvoidanceMomentumController(
    ...
    def __init__(self,
        ...
        Phi = BarrierPotential(proportional_gain=proportional_gain,
                               ...
                               slope_order=1)
        del_Phi = Phi.grad
        super(ObstacleAvoidanceMomentumController,
