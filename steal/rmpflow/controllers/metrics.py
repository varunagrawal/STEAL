...
class IdentityMetric(Metric):
    ...
    def __call__(self, q, qd=None):
        n, d = q.size()
        metric = torch.eye(d, device=self.device).repeat(n, 1,
                                                         1) * self.scaling
        return metric
...
class StretchMetric(Metric):
    ...
    def __call__(self, q, qd=None):
        ...
        metric = torch.einsum('ij,bjk,lk->bil', self.rotation_matrix, S,
...
class ScaledUniformMetric(Metric):
    ...
    def __call__(self, x, xd=None):
        n, d = x.shape
        beta = torch.exp(-torch.norm(x, dim=1).reshape(-1, 1)**2 /
                         (2.0 * self.sigma_**2))
        w = (self.w_u_ - self.w_l_) * beta + self.w_l_
        G = torch.eye(d, device=self.device).repeat(n, 1, 1) * w.repeat(
            1, d**2).reshape(n, d, d)
        return G
...
class DirectionallyStretchedMetric(Metric):
    ...
    def __call__(self, x, xd=None):
        ...
        gamma = max_weight * 1. / torch.cosh(
            x_norm * self.sigma_gamma)**2 + min_weight
        alpha = max_weight * 1. / torch.cosh(
            x_norm * self.sigma_alpha)**2 + min_weight
...
class BarrierMetric(Metric):
    ...
    def __call__(self, x, xd=None):
        return (self.barrier_scalar(self.scale * x) +
                self.epsilon).unsqueeze(2)
