...
from steal.rmpflow.rmp_tree import Rmp
...
class NaturalGradientDescentForceController(Rmp):
    ...
    def __init__(self,
        ...
        elif isinstance(self.B, float):  # scalar
            self.damping_force = lambda x, xd: self.B * xd
        else:  # 3-D tensor function
            self.damping_force = lambda x, xd: torch.einsum(
                'bij,bj->bi', self.B(x, xd), xd)

        if self.del_Phi is None:
            assert self.Phi is not None, ValueError(
                'Phi has to be specified if del_Phi isnt available')
    ...
    def find_curvature_terms(self, x, xd):
        ...
        if G_m.requires_grad:
            ...
            mask = torch.eye(d**2, device=self.device).repeat_interleave(n,
                                                                         dim=0)

            # Finding curvature mass
            dgdxd, = autograd.grad(gmf,
                                   xd_m,
                                   mask,
                                   create_graph=True,
                                   allow_unused=True,
