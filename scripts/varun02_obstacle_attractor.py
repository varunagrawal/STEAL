...
from steal.rmpflow.controllers import (ObstacleAvoidanceForceController,
...
from steal.rmpflow.utils import plot_traj_2D, plt

torch.set_default_dtype(torch.float32)

cspace_dim = 2
wspace_dim = 2
obstacle_center = torch.asarray([[0, 0]])
obstacle_radius = 1.0
obs_alpha = 1e-5
...
# obstacle
obstacle_taskmap = SphereDistanceTaskMap(n_inputs=wspace_dim,
...
circle = plt.Circle((obstacle_center[0, 0], obstacle_center[0, 1]),
                    obstacle_radius,
                    color='k',
                    fill=False)
